...
Therefore the goal of this service is to have a single piece of code taking care of his logic, hiding all technical details, in order to both: 
...
- create a subscriber with a proper spec and store it as an instance variable
- subscribe on open
- unsubscribe on close and on destroy
...
In addition to filtering the events, the service checks the mode of the widget (should auto close, always or not, when clicking inside, when clicking outside, etc.) and then depending on the mode will filter using the event's target.

At the lowest level, a subscriber looks like that: 
...
- `shouldAutoClose`: return `false` to deactivate auto closing, `true` to activate it; subsequent filters will then be called to actually determine if it should be closed or not
...
- `isTargetTogglingElement`: return `true` if you consider the given element is the toggling element (in this case we leave the closing to you), `false` otherwise

While this gives you great flexibility (and you can adapt the semantics to your specific needs), there is a common pattern which emerged and would lead to repeating the same code again and again. Therefore, we provide a facility to create such a structure above, but form a higher level one: 

- delegated properties: `keyEvent`, `mouseEvent`, `close`
...
- consider that the overlay exclusively is the inside
- the rest is the outside (including the popup)
...
- `CallbackPayload`: the object passed to callbacks used to check if the content should be closed or not (see `SubscriptionSpec`)
	- `event`: the DOM event upon which the check occurs
- `Subscription`: what is returned when subscribing; it is a function used to unsubscribe, it's a facilitation 

### Facilitation

- usually widgets have an input property used to tell if auto close should be on or no, and if so in which mode
	- `AutoCloseMode`: if this property is of this type, auto close is on but only for <kbd>escape</kbd> and mouse events occurring either `inside` or `outside` as determined by the given value
	- `AutoCloseType`: otherwise the property can also be a simple boolean, turning off the feature if `false` or turning it on for all events if `true`
- `SubscriptionSpecFactorySpec`: a simplified, higher-level subscription spec, used to generate a lower level one; the goal is to avoid writing the same code again for similar use cases
...
- `Subscriber`: wraps a subscription spec and provides utilities to subscribe, unsubscribe, toggle the subscription, all of that using the same subscription spec and with proper state checking (to avoid subscribing multiple times and so on)
	...
	- the object itself is a function calling `toggle`
...
- `subscribe(subscriptionSpec)`: registers the given subscription, putting it first
- `unsubscribe(subscriptionSpec)`: removes given subscription
- `createSubscriber(subscriptionSpec)`: creates a `Subscriber` from given `SubscriptionSpec`

### Facilitation

- `subscriptionSpecFactory(subscriptionSpecFactorySpec)`: creates a `SubscriptionSpec` from a `SubscriptionSpecFactorySpec`
- `isTargetTogglingElementFactory(getTogglingElement)`: creates a `isTargetTogglingElement` function from one which gets this element
...
- `shouldCloseFactory(getAutoClose)`: creates `shouldClose`, `shouldCloseOnEscape`, `shouldCloseOnClickOutside`, `shouldCloseOnClickInside` functions from one which returns the auto close property which can be `true`, `'inside'` or `'outside'`
...
- `safeElementContains(element, descendant)`: returns `false` if any of the two given elements are not defined, otherwise uses `element.contains(descendant)`
...
When one of this event is triggered, then the filtering occurs (global one and per subscription). So it needs to be efficient.

There is room for optimization, like for instance listening only if necessary, but this must be done with great care since the events handling (see below) for now relies on listening to all of these events.

## Subscriptions management

Subscriptions specs are stored as is, the object passed by the user is not replaced nor modified, it is used as an id for the subscription. Therefore it is important the users keep them, or use the subscriber utility, which keeps them.

This also means that default values are not computed once and for all, and the spec could be changed dynamically (by mutating the object) to alter the whole behavior. However this is an unlikely use case, since this would mean the nature of the widget itself changes.

There is room for optimization, for instance by considering the spec as immutable we could optimize the default value. But in this case, if users want to change the spec — again, this is very unlikely — we should provide a way to replace it.

Regarding the data structure which is used, it's the equivalent of a stack. It is very important to process the subscriptions in the reverse order of their registration, because we stop processing subscriptions for one user action when one subscription has been called. By processing in this order, we can handle the case of nested popups, where the innermost popups are registered last, and processed first. So for instance an opened dropdown inside a modal dialog will be closed on first <kbd>escape</kbd>, and the modal will be closed on the second <kbd>escape</kbd>.

## Events handling: subscriptions calls

Upon every listened events checks are done: 

- global checks, not related to any subscribers; to listen to <kbd>escape</kbd> only, left-click only, etc.
- checks per subscriptions

This is described in the suer documentation therefore no need to tell more about it here.

There is on technical aspect to be described though: the processing of the __same user action__. A single user action triggers a sequence of events, usually of the same type. Therefore, our check routine will be called upon every of those events. However we want to handle all of these events as a whole — as one single user action — for this sole requirement described previously: stop processing more subscribers when one has been called.

For that, luckily our class is a singleton and there can be only one single user action at a time: this way we can store an internal state to check what has been done during handle of previous events during the same user action.

So we reset the state when handling the first event of the sequence (for instance `mousedown` for mouse events), and at the end of each handler we store the state, which in our case just tells whether a subscription has already been called or not. This state can then be checked to skip the subscriptions checking.
